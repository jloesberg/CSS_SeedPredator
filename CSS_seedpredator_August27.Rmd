...
```{r}
# Selecting exclusion sites with 5 or less seeds missing, there are 94
exc <- seed %>% 
   filter(Treatment == "C") %>% 
  filter(Total_eaten <=5)
...
# BR and A only sites with seed eaten no. between 10 and 70
BR <- seed %>% 
  filter(Treatment == "BR+") %>% 
...
# These are the sites that do not have camera data: let's exclude them from the analyses (unclick these if you want to include them)
...
Does Bromus differ from 0.25? yep
```{r}
bromus <- Global_final %>% 
...
```{r}
labels <- c(ant = "Ant", CA_Towee = "California Towhee", GS = "Ground Squirrel", mixed_bird = "Multiple Bird", mixed_mammal = "Multiple Mammal", multipred = "Multiple Taxa", none = "No Visits", scrub_jay = "Scrub Jay", Stellar_Jay = "Stellar Jay", WC_sparrow = "White Crowned Sparrow", woodrat = "Woodrat")
...
#ggsave("figures/specices_preference.png", width = 7, height = 5 )
...
#tbl1 %>% gtsave("figures/fig3_Sept10.png")
```
